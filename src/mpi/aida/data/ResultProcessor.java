...
public class ResultProcessor {
  ...
  public ResultProcessor(DisambiguationResults result, String inputFile, PreparedInput input) {
  ...
  private String annotateInputText(String text, DisambiguationResults results) {
    // AIDA treats [[...]] as special markup that needs to be removed in the output.
    String cleanedText = text.replaceAll("\\[\\[([^\\]]*)\\]\\]", "$1");
    ...
    while(current<len){
      if(mentionIndex.containsKey(current)){
        ...
        sBuff.append(cleanedText.substring(start, current));
