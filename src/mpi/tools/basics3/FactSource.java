...
public abstract class FactSource implements Iterable<Fact> {
	...
	public static FactSource from(String f) throws MalformedURLException {
		if (f.startsWith("http:"))
			return (new UrlFactSource(new URL(f)));
	...
	protected static Iterator<Fact> factReader(Reader reader,
	...
	public static class FileFactSource extends FactSource {
		...
		public Iterator<Fact> iterator() {
			try {
				return factReader(FileUtils.getBufferedUTF8Reader(file),
						FileSet.extension(file));
			} catch (Exception e) {
	...
	protected static class UrlFactSource extends FactSource {
		...
		public Iterator<Fact> iterator() {
			try {
				return factReader(
						FileUtils.getBufferedUTF8Reader(file.openStream()),
						FileSet.extension(file.toString()));
			} catch (Exception e) {
				e.printStackTrace();
				return (null);
			}
	...
	public static void main(String[] args) throws Exception {
		for (Fact f : FactSource
				.from("c:/fabian/data/yago2s/yagoMetaFacts.tsv")) {
			D.p(f);
		}
	}
