...
public class FactComponent {
	...
	public static final Map<String, String> standardPrefixes = new FinalMap<String, String>(
			"rdf:", "http://www.w3.org/1999/02/22-rdf-syntax-ns#", "rdfs:",
			"http://www.w3.org/2000/01/rdf-schema#", "xsd:",
			"http://www.w3.org/2001/XMLSchema#", "owl:",
			"http://www.w3.org/2002/07/owl#", "skos:",
			"http://www.w3.org/2004/02/skos/core#", "dbp:",
		...
		if (s.startsWith("http://")) {
			for (Entry<String, String> entry : standardPrefixes.entrySet()) {
				if (s.startsWith(entry.getValue())) {
					return (forQname(entry.getKey(),
							s.substring(entry.getValue().length())));
				}
			}
		}
	...
	public static String forAny(String s) {
		...
		if (s.indexOf(':') != -1) {
			return (forQname(s.substring(0, s.indexOf(':') + 1),
	...
	public static String setDataType(String stringLiteral, String datatype) {
		...
		if (split == null)
			return (null);
		if (D.equal(split[1], datatype))
			return (stringLiteral);
		return (FactComponent.forStringWithDatatype(stripQuotes(split[0]),
						datatype));
	}
	...
	public static String setLanguage(String stringLiteral, String language) {
		...
		if (split == null)
			return (null);
		if (D.equal(split[2], language))
			return (stringLiteral);
		return (FactComponent.forStringWithLanguage(stripQuotes(split[0]),
						language));
	}
  ...
  public static boolean isCategory(String cat) {
    cat = stripBracketsAndLanguage(cat);
    if (cat.startsWith("wikicat_") || cat.startsWith("wordnet_") )
      return true;
    else 
      return false;
  }
	...
	public static String hash(String string) {
		...
		return (Long.toString((long) hash - (long) Integer.MIN_VALUE,
						Character.MAX_RADIX));
	}
	...
	public static String wordnetWord(String wordnetEntity) {
		if (!wordnetEntity.startsWith("<wordnet_")
				|| wordnetEntity.length() < 8 + 9)
			return (null);
		wordnetEntity = wordnetEntity.substring("<wordnet_".length(),
				wordnetEntity.length() - 11);
	...
	public static String dbpediaClassForYagoClass(String arg) {
		...
		if (arg.startsWith("wordnet_"))
			arg = arg.substring("wordnet_".length());
		if (arg.startsWith("wikicategory_"))
			arg = arg.substring("wikicategory_".length());
		...
		for (int i = 0; i < arg.length(); i++) {
			...
			result.append(upcase ? Character.toUpperCase(arg.charAt(i)) : arg
					.charAt(i));
			upcase = false;
		}
	...
	public static String stripClass(String cls) {
		...
		if (strippedCls.startsWith("wordnet_")) {
			strippedCls = strippedCls.substring("wordnet_".length());
			strippedCls = strippedCls.substring(strippedCls.lastIndexOf("_") - 1);
